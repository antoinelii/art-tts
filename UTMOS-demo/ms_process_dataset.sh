...
for dataset in pb2007; do
	for version in v6; do
		for E in 200 500 1000 2000 3000; do
			CKPT_NAME=grad_${E}
			utmos_version_dir=${MAIN_DATA_DIR}/VoxCommunis/${dataset}/UTMOS_data/decoder_files_${version}/${CKPT_NAME}
			mkdir -p  ${utmos_version_dir}
			find ${MAIN_DATA_DIR}/VoxCommunis/${dataset} -type f -path "*/hifigan_pred/${version}/${CKPT_NAME}/*_decoder.wav" \
