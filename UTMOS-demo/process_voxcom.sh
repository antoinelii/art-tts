...
split=test-20h
for version in v6_zhCN; do
	for E in 5000; do
		CKPT_NAME=grad_${E}
		utmos_version_dir=${MAIN_DATA_DIR}/${dataset}/UTMOS_data/decoder_files_${version}/${split}/${CKPT_NAME}/${generator_version}
		mkdir -p  ${utmos_version_dir}
		find ${MAIN_DATA_DIR}/${dataset}/${split} -type f -path "*/hifigan_pred/${version}/${CKPT_NAME}/${generator_version}/*_decoder.wav" \
