...
#for dataset in LJSpeech-1.1 MNGU0 mocha_timit MSPKA_EMA_ita; do
for dataset in MNGU0 mocha_timit MSPKA_EMA_ita pb2007; do
	for version in msml1h; do
	#for version in sparc; do
		for E in 500 1000; do
		#for E in 200; do	
			CKPT_NAME=grad_${E}
			#utmos_version_dir=${MAIN_DATA_DIR}/${dataset}/UTMOS_data/decoder_files_${version}/${CKPT_NAME}
			utmos_version_dir=${MAIN_DATA_DIR}/VoxCommunis/${dataset}/UTMOS_data/decoder_files_${version}/${CKPT_NAME}/sparc_multi
			mkdir -p  ${utmos_version_dir}
			find ${MAIN_DATA_DIR}/VoxCommunis/${dataset} -type f -path "*/hifigan_pred/${version}/${CKPT_NAME}/sparc_multi/*_decoder.wav" \
				-exec cp {} ${utmos_version_dir} \;
			#find ${MAIN_DATA_DIR}/VoxCommunis/${dataset} -type f -path "*/hifigan_pred/${version}/sparc_multi/*.wav" \
			#	-exec cp {} ${utmos_version_dir} \;
