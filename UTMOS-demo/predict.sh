...
for dataset in LJSpeech-1.1 MNGU0 mocha_timit MSPKA_EMA_ita; do
	for version in src sparc v1 v1_1 v2 v4; do
		uv run predict.py --mode predict_dir \
				--inp_dir ${MAIN_DATA_DIR}/${dataset}/UTMOS_data/decoder_files_${version} \
				--bs 32 \
				--out_path ${MAIN_DATA_DIR}/${dataset}/UTMOS_data/decoder_files_${version}/utmos.csv
	done
done
