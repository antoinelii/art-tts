#!/bin/bash
#SBATCH --job-name=vocoder_infer     # Job name
...
for E in 2000 4000 6000 8000 10000; do
    ...
    for MODEL_VERSION in v2_full; do
        if [ "$MODEL_VERSION" = "v2" ]; then
            ...
            FILELIST_VERSION=v1
        elif [ "$MODEL_VERSION" = "v2_full" ]; then
            FILELIST_VERSION=v2
        ...
        if [ "$DATASET" = "LJSpeech-1.1" ]; then
            SRC_ART=decoder
            echo "Running vocoder inference for dataset: $DATASET, from : $SRC_ART"
            srun python -u ./scripts/inference/vocoder_inference.py  --data_dir ${MAIN_DATA_DIR}/LJSpeech-1.1/arttts_pred/${MODEL_VERSION}/${CKPT_NAME} \
            ...
            srun python -u ./scripts/inference/vocoder_inference.py  --data_dir ${MAIN_DATA_DIR}/$DATASET/arttts/$SPK/arttts_pred/${MODEL_VERSION}/${CKPT_NAME} \
