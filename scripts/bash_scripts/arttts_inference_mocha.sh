...
for E in 2000 4000 6000 8000 10000; do
    ...
    for MODEL_VERSION in v2_full; do
        if [ "$MODEL_VERSION" = "v1" ]; then
            ...
            FILELIST_VERSION=v1
        elif [ "$MODEL_VERSION" = "v2" || "$MODEL_VERSION" = "v2_phnmtext" ]; then
            FILELIST_VERSION=v2
        elif [ "$MODEL_VERSION" = "v3" ]; then
            FILELIST_VERSION=v1
        elif [ "$MODEL_VERSION" = "v4" || "$MODEL_VERSION" = "v4_phnmtext" ]; then
            FILELIST_VERSION=v4
        ...
        if [ "$DATASET" = "LJSpeech-1.1" ]; then
            ...
            srun python -u ./scripts/inference/arttts_inference.py  --data_dir ${MAIN_DATA_DIR}/ \
            ...
            #srun python -u ./scripts/inference/arttts_inference.py  --data_dir ${MAIN_DATA_DIR} \
            #                                    --save_dir ${MAIN_DATA_DIR}/$DATASET/arttts/$SPK/arttts_pred/${MODEL_VERSION}/${CKPT_NAME}_align \
            #                                    --filelist_path resources/filelists/$DATASET/${SPK}_${FILELIST_VERSION}.txt \
            #                                    --version ${MODEL_VERSION} \
            #                                    --ckpt_name ${CKPT_NAME}.pt \
            #                                    --params_name params_${MODEL_VERSION} \
