...
#SBATCH --array=0-5 # job array with 4 tasks (0 to 3)
...
DATASET=mocha_timit # MNGU0, mocha_timit, MSPKA_EMA_ita, pb2007
CKPT_NAME=grad_5000
MODEL_VERSION=v1
FILELIST_VERSION=v1
...
for E in 200 1000 2000 3000 4000 5000; do
    ...
    for MODEL_VERSION in v4; do
        ...
        if [ "$DATASET" = "LJSpeech-1.1" ]; then
            ...
            #srun python -u ./scripts/inference/hifigan_inference.py  --data_dir ${MAIN_DATA_DIR}/LJSpeech-1.1/encoded_audio_en/emasrc \
            ...
            srun python -u ./scripts/inference/hifigan_inference.py  --data_dir ${MAIN_DATA_DIR}/LJSpeech-1.1/arttts_pred/${MODEL_VERSION}/${CKPT_NAME} \
            ...
            #echo "Running hifigan inference for dataset: $DATASET, from SPARC encoded"
            #srun python -u ./scripts/inference/hifigan_inference.py  --data_dir ${MAIN_DATA_DIR}/$DATASET/arttts/$SPK/encoded_audio_en/emasrc \
            ...
            srun python -u ./scripts/inference/hifigan_inference.py  --data_dir ${MAIN_DATA_DIR}/$DATASET/arttts/$SPK/arttts_pred/${MODEL_VERSION}/${CKPT_NAME} \
