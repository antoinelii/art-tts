...
for E in 4000; do
    ...
    for MODEL_VERSION in msml1h; do
        for CUSTOM_DATASET in MNGU0 MSPKA_EMA_ita pb2007 mocha_timit ; do
            echo "Computing PCCs for dataset: $CUSTOM_DATASET, src_art : $MODEL_VERSION $CKPT_NAME decoder"
            srun python -u ./scripts/misc/quanti_art_voxcom_custom.py  --main_data_dir ${MAIN_DATA_DIR} \
                                                --dataset ${CUSTOM_DATASET} \
                                                --preds_dir ${MAIN_DATA_DIR}/${DATASET}/${CUSTOM_DATASET}/arttts_pred/${MODEL_VERSION}/${CKPT_NAME} \
                                                --save_dir ${MAIN_DATA_DIR}/${DATASET}/${CUSTOM_DATASET}/analysis/ \
