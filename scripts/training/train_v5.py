...
from art_tts.configs import params_v5
from art_tts.model import AttentionTTS
...
log_dir = params_v5.log_dir
# profile_log_dir = log_dir + "_profiler"
reorder_feats = params_v5.reorder_feats
...
val_every = params_v5.val_every
save_every = params_v5.save_every
...
if __name__ == "__main__":
    torch.manual_seed(params_v5.random_seed)
    np.random.seed(params_v5.random_seed)
    ...
    logger = SummaryWriter(log_dir=params_v5.log_dir)

    mylogger.info("Initializing data loaders...")
    train_dataset = PhnmArticDataset(
        params_v5.train_filelist_path,
        data_root_dir=params_v5.data_root_dir,
        reorder_feats=params_v5.reorder_feats,
        pitch_idx=params_v5.pitch_idx,
        loudness_idx=params_v5.loudness_idx,
        log_normalize_loudness=params_v5.log_normalize_loudness,
        merge_diphtongues=params_v5.merge_diphtongues,
        load_coder=False,
        sparc_ckpt_path=params_v5.sparc_ckpt_path,
        shuffle=params_v5.shuffle,  # do not shuffle for compatibility custom sampler
        random_seed=params_v5.random_seed,
    )
    train_lengths = get_lengths(train_dataset)
    train_sampler = LengthGroupedSampler(
        batch_size=params_v5.batch_size,
        lengths=train_lengths,
        generator=torch.manual_seed(params_v5.random_seed),
    )
    ...
    model = AttentionTTS(
        params_v5.n_ipa_feats,
        params_v5.n_spks,
        None,
        params_v5.n_enc_channels,
        params_v5.filter_channels,
        params_v5.filter_channels_dp,
        params_v5.n_heads,
        params_v5.n_enc_layers,
        params_v5.enc_kernel,
        params_v5.enc_dropout,
        params_v5.window_size,
        params_v5.n_feats,
        params_v5.dec_dim,
        params_v5.beta_min,
        params_v5.beta_max,
    ...
    if start_epoch == 1:  # start training from scratch
        ...
        early_stopping = torch.load(
            Path(params_v5.log_dir) / "early_stopping.pt", weights_only=False
        )

        mylogger.info(
            f"Loading model state dict from ckpt grad_{start_epoch - 1}.pt ..."
        )
        ckpt_grad = torch.load(Path(params_v5.log_dir) / f"grad_{start_epoch - 1}.pt")
    ...
    optimizer = torch.optim.Adam(params=model.parameters(), lr=params_v5.learning_rate)

    mylogger.info("Logging valid batch...")
    valid_batch = valid_dataset.sample_test_batch(size=params_v5.test_size)
    for i, item in enumerate(valid_batch):
        ...
        save_plot_art_14([art], f"{params_v5.log_dir}/original_{i}.png")

    mylogger.info("Start training...")
    iteration = 0
    best_epoch = 0
    # with tqdm(
    #    range(1, params_v5.n_epochs + 1),
    #    total=params_v5.n_epochs,
    #    desc="Training",
    #    position=1,
    #    dynamic_ncols=True,
    # ) as progress_bar:
        ...
        for epoch in progress_bar:
            ...
            for batch_idx, batch in enumerate(loader):
                ...
                dur_loss, prior_loss, diff_loss = model.compute_loss(
                    x, x_lengths, y, y_lengths, out_size=params_v5.out_size
                )
            ...
            if epoch % val_every == 0:
                ...
                # elif patience_counter >= params_v5.patience:
