...
from art_tts.model.utils import sequence_mask, convert_pad_shape
...
class IpaTraitEncoder(BaseModule):
    def __init__(
        ...
        # here chose to reduce last dimension to n_ipa_feats to preserve residual
        # connection with input dim (though slow dim 25 compared to 192)
        ...
        self.encoder = Encoder(
            # small input dim 25... especially compared to spk_emb_dim 64
            n_ipa_feats + spk_emb_dim,
            filter_channels,
            n_heads,
            n_layers,
            kernel_size,
        ...
        self.proj_m = torch.nn.Conv1d(n_ipa_feats + spk_emb_dim, n_feats, 1)

    def forward(self, x, x_lengths, spk=None):
        x_mask = torch.unsqueeze(sequence_mask(x_lengths, x.size(2)), 1).to(
            x.dtype
        )  # shape: (B, 1, P)
        x = self.prenet(x, x_mask)  # shape: (B, n_ipa_feats, P)
        x = torch.cat([x, spk.unsqueeze(-1).repeat(1, 1, x.shape[-1])], dim=1)
        x = self.encoder(x, x_mask)  # shape: (B, n_ipa_feats, P)
        mu = self.proj_m(x) * x_mask  # shape: (B, n_feats, P)
        return mu, x_mask
