...
class GradTTS(BaseModule):
    ...
    def compute_loss(self, x, x_lengths, y, y_lengths, spk=None, out_size=None):
        ...
        with torch.no_grad():
            ...
            factor = -0.5 * torch.ones(
                mu_x.shape, dtype=mu_x.dtype, device=mu_x.device
            )  # (B, n_feats, T_x)  -0.5 for the inv square root
            y_square = torch.matmul(
                factor.transpose(1, 2), y**2
            )  # (B, T_x, T_y) # sum square along n_feats
            y_mu_double = torch.matmul(
                2.0 * (factor * mu_x).transpose(1, 2), y
            )  # (B, T_x, T_y)  # scalar prod matrix between each frame of mu_x and y
            mu_square = torch.sum(factor * (mu_x**2), 1).unsqueeze(-1)  # (B, T_x, 1)
            log_prior = y_square - y_mu_double + mu_square + const  # (B, T_x, T_y)
...
class AttentionTTS(BaseModule):
    def __init__(
        ...
        super(AttentionTTS, self).__init__()
        self.n_ipa_feats = n_ipa_feats
        self.n_spks = n_spks
        self.spk_emb_dim = spk_emb_dim
        self.n_enc_channels = n_enc_channels
...
class AttentionTTSPreblock(BaseModule):
    def __init__(
        self,
        n_ipa_feats,
        n_spks,
        ...
        super(AttentionTTSPreblock, self).__init__()
        self.n_ipa_feats = n_ipa_feats
        self.n_spks = n_spks
        self.spk_emb_dim = spk_emb_dim
        self.n_enc_channels = n_enc_channels
        ...
        self.decoder = Diffusion1DPreblock(
            n_feats, dec_dim, n_spks, spk_emb_dim, beta_min, beta_max, pe_scale
        )
