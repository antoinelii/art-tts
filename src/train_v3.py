...
val_every = params_v3.val_every
save_every = params_v3.save_every


if __name__ == "__main__":
    torch.manual_seed(params_v3.random_seed)
    np.random.seed(params_v3.random_seed)

    mylogger.info("Initializing logger...")
    logger = SummaryWriter(log_dir=params_v3.log_dir)

    mylogger.info("Initializing data loaders...")
    train_dataset = PhnmMelDataset(
        params_v3.train_filelist_path,
        data_root_dir=params_v3.data_root_dir,
    ...
    batch_collate = PhnmMelBatchCollate()
    loader = DataLoader(
    ...
    valid_dataset = PhnmMelDataset(
        params_v3.valid_filelist_path,
        data_root_dir=params_v3.data_root_dir,
        n_fft=params_v3.n_fft,
        n_mels=params_v3.n_feats,
        sample_rate=params_v3.sample_rate,
    ...
    if start_epoch == 1:  # start training from scratch
        ...
        early_stopping = torch.load(
            Path(params_v3.log_dir) / "early_stopping.pt", weights_only=False
        )

        mylogger.info(
            f"Loading model state dict from ckpt grad_{start_epoch - 1}.pt ..."
        )
        ckpt_grad = torch.load(Path(params_v3.log_dir) / f"grad_{start_epoch - 1}.pt")
    ...
    optimizer = torch.optim.Adam(params=model.parameters(), lr=params_v3.learning_rate)

    mylogger.info("Logging valid batch...")
    valid_batch = valid_dataset.sample_test_batch(size=params_v3.test_size)
    for i, item in enumerate(valid_batch):
        ...
        save_plot(mel.squeeze(), f"{params_v3.log_dir}/original_{i}.png")
        ...
        for epoch in progress_bar:
            ...
            for batch_idx, batch in enumerate(loader):
                ...
                dur_loss, prior_loss, diff_loss = model.compute_loss(
                    x, x_lengths, y, y_lengths, out_size=params_v3.out_size
                )
            ...
            if epoch % val_every == 0:
                ...
                # elif patience_counter >= params_v3.patience:
