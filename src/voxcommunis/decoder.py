...
class UniqueSegmentFeature:
    ...
    def add_segment_feature_batch(
        self, segment_feature_batch: list[tuple[str, list[tuple[int, ...]]]]
    ) -> None:
        for segment, features in segment_feature_batch:
            if len(features) == 1:
                ...
                self.add_segment_feature(
                    segment,
                    tuple(
                        feats[0] if len(set(feats)) == 1 else 0
                        for feats in zip(*features)
                    ),
                )
            else:
...
class FeatureDecoder:
    ...
    @functools.cache
    def segment_id(self, segment: str) -> int:
        ...
        if not any(features):
            ...
            indices = np.flatnonzero(
                np.logical_and.reduce(
                    [
                        feat2idx[f]
                        for feat2idx, f in zip(self.feature_to_indices, features)
                        if f != 0
                    ]
                )
            )
