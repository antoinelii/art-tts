...
from art_tts.configs import params_v5_preblock
from art_tts.model import AttentionTTSPreblock
...
log_dir = params_v5_preblock.log_dir
# profile_log_dir = log_dir + "_profiler"
reorder_feats = params_v5_preblock.reorder_feats
...
val_every = params_v5_preblock.val_every
save_every = params_v5_preblock.save_every
...
if __name__ == "__main__":
    torch.manual_seed(params_v5_preblock.random_seed)
    np.random.seed(params_v5_preblock.random_seed)
    ...
    logger = SummaryWriter(log_dir=params_v5_preblock.log_dir)

    mylogger.info("Initializing data loaders...")
    train_dataset = PhnmArticDataset(
        params_v5_preblock.train_filelist_path,
        data_root_dir=params_v5_preblock.data_root_dir,
        reorder_feats=params_v5_preblock.reorder_feats,
        pitch_idx=params_v5_preblock.pitch_idx,
        loudness_idx=params_v5_preblock.loudness_idx,
        log_normalize_loudness=params_v5_preblock.log_normalize_loudness,
        merge_diphtongues=params_v5_preblock.merge_diphtongues,
        load_coder=False,
        sparc_ckpt_path=params_v5_preblock.sparc_ckpt_path,
        shuffle=params_v5_preblock.shuffle,  # do not shuffle for compatibility custom sampler
        random_seed=params_v5_preblock.random_seed,
    )
    train_lengths = get_lengths(train_dataset)
    train_sampler = LengthGroupedSampler(
        batch_size=params_v5_preblock.batch_size,
        lengths=train_lengths,
        generator=torch.manual_seed(params_v5_preblock.random_seed),
    )
    ...
    loader = DataLoader(
        ...
        num_workers=3,
        shuffle=False,
        sampler=train_sampler,  # Use custom sampler for balanced batches
        pin_memory=True,
    )
    valid_dataset = PhnmArticDataset(
        params_v5_preblock.valid_filelist_path,
        data_root_dir=params_v5_preblock.data_root_dir,
        reorder_feats=params_v5_preblock.reorder_feats,
        pitch_idx=params_v5_preblock.pitch_idx,
        loudness_idx=params_v5_preblock.loudness_idx,
    ...
    val_lengths = get_lengths(valid_dataset)
    ...
    val_loader = DataLoader(
        ...
        num_workers=3,
        shuffle=False,
        sampler=val_sampler,  # Use custom sampler for balanced batches
        pin_memory=True,
    )

    mylogger.info("Initializing model...")
    model = AttentionTTSPreblock(
        params_v5_preblock.n_ipa_feats,
        params_v5_preblock.n_spks,
        None,
        params_v5_preblock.n_enc_channels,
        params_v5_preblock.filter_channels,
        params_v5_preblock.filter_channels_dp,
        params_v5_preblock.n_heads,
        params_v5_preblock.n_enc_layers,
        params_v5_preblock.enc_kernel,
        params_v5_preblock.enc_dropout,
        params_v5_preblock.window_size,
        params_v5_preblock.n_feats,
        params_v5_preblock.dec_dim,
        params_v5_preblock.beta_min,
        params_v5_preblock.beta_max,
        params_v5_preblock.pe_scale,
    ).to(device)
    ...
    if start_epoch == 1:  # start training from scratch
        ...
        early_stopping = torch.load(
            Path(params_v5_preblock.log_dir) / "early_stopping.pt", weights_only=False
        )
        ...
        ckpt_grad = torch.load(
            Path(params_v5_preblock.log_dir) / f"grad_{start_epoch - 1}.pt"
        )
    ...
    optimizer = torch.optim.Adam(
        params=model.parameters(), lr=params_v5_preblock.learning_rate
    )

    mylogger.info("Logging valid batch...")
    valid_batch = valid_dataset.sample_test_batch(size=params_v5_preblock.test_size)
    for i, item in enumerate(valid_batch):
        ...
        save_plot_art_14([art], f"{params_v5_preblock.log_dir}/original_{i}.png")

    mylogger.info("Start training...")
    iteration = 0
    best_epoch = 0
    # with tqdm(
    #    range(1, params_v5_preblock.n_epochs + 1),
    #    total=params_v5_preblock.n_epochs,
    #    desc="Training",
    #    position=1,
    #    dynamic_ncols=True,
    # ) as progress_bar:
    with tqdm(
        ...
        for epoch in progress_bar:
            ...
            #    schedule=schedule(wait=100, warmup=1, active=3, repeat=1),
            #    on_trace_ready=torch.profiler.tensorboard_trace_handler(
            #        profile_log_dir, worker_name=f"epoch_{epoch}"
            #    ),
            #    record_shapes=True,
            #    profile_memory=True,
            #    with_stack=True,
            ...
            for batch_idx, batch in enumerate(loader):
                ...
                dur_loss, prior_loss, diff_loss = model.compute_loss(
                    x, x_lengths, y, y_lengths, out_size=params_v5_preblock.out_size
                )
            ...
            # Print profiler results for debugging
            # mylogger.info(profiler.key_averages().table(sort_by="cuda_time_total", row_limit=10))
            ...
            if epoch % val_every == 0:
                ...
                for batch_idx, batch in enumerate(val_loader):
                    with torch.no_grad():
                        ...
                        dur_loss, prior_loss, diff_loss = model.compute_loss(
                            ...
                            y,
                            y_lengths,
                            out_size=params_v5_preblock.out_size,
                        )
                ...
                # elif patience_counter >= params_v5_preblock.patience:
