...
start_epoch = 1
end_epoch = 5000
val_every = params_v6_zhCN.val_every
save_every = params_v6_zhCN.save_every


def get_train_dataset(tokenizer):
    train_dataset = PhnmArticDataset(
        dataset_dir=params_v6_zhCN.dataset_dir,
        separate_files=params_v6_zhCN.separate_files,
        manifest_path=params_v6_zhCN.train_manifest,
        alignment_path=params_v6_zhCN.train_alignment,
        feature_tokenizer=tokenizer,
        reorder_feats=params_v6_zhCN.reorder_feats,
        pitch_idx=params_v6_zhCN.pitch_idx,
        loudness_idx=params_v6_zhCN.loudness_idx,
        log_normalize_loudness=params_v6_zhCN.log_normalize_loudness,
        random_seed=params_v6_zhCN.random_seed,
    )
...
def get_valid_dataset(tokenizer):
    valid_dataset = PhnmArticDataset(
        dataset_dir=params_v6_zhCN.dataset_dir,
        separate_files=params_v6_zhCN.separate_files,
        manifest_path=params_v6_zhCN.val_manifest,
        alignment_path=params_v6_zhCN.val_alignment,
        feature_tokenizer=tokenizer,
        reorder_feats=params_v6_zhCN.reorder_feats,
        pitch_idx=params_v6_zhCN.pitch_idx,
        loudness_idx=params_v6_zhCN.loudness_idx,
...
def init_data_loader(
    ...
    if validation:
        loader = DataLoader(
            dataset=dataset,
            batch_size=params_v6_zhCN.batch_size,
        ...
        if multilingual:
            sampler = LengthGroupedLanguageUpSampler(
                ...
                generator=torch.manual_seed(params_v6_zhCN.random_seed),
                upsample_factor=0.5,
            )
        else:
...
def log_valid_batch(logger, valid_dataset):
    """done only on rank 0"""
    valid_batch = valid_dataset.sample_test_batch(size=params_v6_zhCN.test_size)
    for i, item in enumerate(valid_batch):
        ...
        save_plot_art_14([art], f"{params_v6_zhCN.log_dir}/original_{i}.png")
    return valid_batch


def init_model():
    model = GradTTArtic(
        n_ipa_feats=params_v6_zhCN.n_ipa_feats,  # 26, 24 phonological traits + 1 silence dim + 1 phoneme repetition count
        spk_emb_dim=params_v6_zhCN.spk_emb_dim,  # 64
        n_enc_channels=params_v6_zhCN.n_enc_channels,  # 192
        filter_channels=params_v6_zhCN.filter_channels,  # 768
        filter_channels_dp=params_v6_zhCN.filter_channels_dp,  # 256
        n_heads=params_v6_zhCN.n_heads,  # 2
        n_enc_layers=params_v6_zhCN.n_enc_layers,  # 6
        enc_kernel=params_v6_zhCN.enc_kernel,  # 3
        enc_dropout=params_v6_zhCN.enc_dropout,  # 0.1
        window_size=params_v6_zhCN.window_size,  # 4
        n_feats=params_v6_zhCN.n_feats,  # 16 (articulatory features)
        dec_dim=params_v6_zhCN.dec_dim,  # 64
        beta_min=params_v6_zhCN.beta_min,  # 0.05
        beta_max=params_v6_zhCN.beta_max,  # 20.0
        pe_scale=params_v6_zhCN.pe_scale,  # 1000
...
def train_loop(model, train_loader, optimizer, epoch):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        prior_loss, diff_loss = model.compute_loss(
            x, x_lengths, y, y_lengths, spk_ft, out_size=params_v6_zhCN.out_size
        )
...
def train():
    ...
    logger = SummaryWriter(log_dir=params_v6_zhCN.log_dir)
    ...
    train_loader = init_data_loader(
        train_dataset,
        validation=False,
        multilingual=params_v6_zhCN.separate_files,
    )
    ...
    optimizer = torch.optim.Adam(
        params=model.parameters(), lr=params_v6_zhCN.learning_rate
    )
...
if __name__ == "__main__":
    torch.manual_seed(params_v6_zhCN.random_seed)
    np.random.seed(params_v6_zhCN.random_seed)
