...
if __name__ == "__main__":
    ...
    if start_epoch == 1:  # start training from scratch
        ...
        early_stopping = torch.load(
            Path(params_v1_1.log_dir) / "early_stopping.pt", weights_only=False
        )

        mylogger.info(
            f"Loading model state dict from ckpt grad_{start_epoch - 1}.pt ..."
        )
        ckpt_grad = torch.load(Path(params_v1_1.log_dir) / f"grad_{start_epoch - 1}.pt")
    ...
    best_epoch = 0
    # with tqdm(
    #    range(1, params_v1_1.n_epochs + 1),
    #    total=params_v1_1.n_epochs,
    #    desc="Training",
    #    position=1,
    #    dynamic_ncols=True,
    # ) as progress_bar:
    with tqdm(
        ...
        for epoch in progress_bar:
            ...
            if epoch % val_every == 0:
                ...
                # elif patience_counter >= params_v1_1.patience:
