...
This code is taken from https://github.com/Berkeley-Speech-Group/Speech-Articulatory-Coding/blob/main/sparc/generator.py
...
class HiFiGANGenerator(torch.nn.Module):
    ...
    def __init__(
        ...
        assert len(upsample_scales) == len(upsample_kernel_sizes), (
            f"{upsample_scales} | {upsample_kernel_sizes}"
        )
        ...
        if paddings is None:
            ...
            for i, s in enumerate(paddings):
                if s == "default":
                    new_paddings.append(
                        upsample_scales[i] // 2 + upsample_scales[i] % 2
                    )
                else:
    ...
    def forward(self, c, spk_emb=None, ar=None, **kwargs):
        ...
        """
        c[:, self.pitch_axis] = (
            c[:, self.pitch_axis] - self.pitch_offset
        ) * self.pitch_rescale
        c = self.input_conv(c)
