...
from art_tts.configs import params_msml1h
...
log_dir = Path(params_msml1h.log_dir)
...
start_epoch = 1
end_epoch = 5000
val_every = params_msml1h.val_every
save_every = params_msml1h.save_every


def get_train_dataset(tokenizer):
    train_dataset = PhnmArticDataset(
        dataset_dir=params_msml1h.dataset_dir,
        separate_files=params_msml1h.separate_files,
        manifest_path=params_msml1h.train_manifest,
        alignment_path=params_msml1h.train_alignment,
        ...
        exclude_langs=params_msml1h.exclude_langs,
    )
...
def get_valid_dataset(tokenizer):
    valid_dataset = PhnmArticDataset(
        dataset_dir=params_msml1h.dataset_dir,
        separate_files=params_msml1h.separate_files,
        manifest_path=params_msml1h.val_manifest,
        alignment_path=params_msml1h.val_alignment,
        feature_tokenizer=tokenizer,
        reorder_feats=params_msml1h.reorder_feats,
        pitch_idx=params_msml1h.pitch_idx,
        loudness_idx=params_msml1h.loudness_idx,
    ...
    if validation:
        ...
        if multilingual:
            sampler = LengthGroupedLanguageUpSampler(
                ...
                upsample_factor=params_msml1h.upsample_factor,
            )
        else:
...
def log_valid_batch(logger, valid_dataset):
    """done only on rank 0"""
    valid_batch = valid_dataset.sample_test_batch(size=params_msml1h.test_size)
    for i, item in enumerate(valid_batch):
        ...
        save_plot_art_14([art], f"{params_msml1h.log_dir}/original_{i}.png")
    return valid_batch


def init_model():
    model = GradTTArtic(
        n_ipa_feats=params_msml1h.n_ipa_feats,  # 26, 24 phonological traits + 1 silence dim + 1 phoneme repetition count
        spk_emb_dim=params_msml1h.spk_emb_dim,  # 64
        n_enc_channels=params_msml1h.n_enc_channels,  # 192
        filter_channels=params_msml1h.filter_channels,  # 768
        filter_channels_dp=params_msml1h.filter_channels_dp,  # 256
        n_heads=params_msml1h.n_heads,  # 2
        n_enc_layers=params_msml1h.n_enc_layers,  # 6
        enc_kernel=params_msml1h.enc_kernel,  # 3
        enc_dropout=params_msml1h.enc_dropout,  # 0.1
        window_size=params_msml1h.window_size,  # 4
        n_feats=params_msml1h.n_feats,  # 16 (articulatory features)
        dec_dim=params_msml1h.dec_dim,  # 64
        beta_min=params_msml1h.beta_min,  # 0.05
        beta_max=params_msml1h.beta_max,  # 20.0
        pe_scale=params_msml1h.pe_scale,  # 1000
...
def train_loop(model, train_loader, optimizer, epoch):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        prior_loss, diff_loss = model.compute_loss(
            x, x_lengths, y, y_lengths, spk_ft, out_size=params_msml1h.out_size
        )
...
def train():
    ...
    logger = SummaryWriter(log_dir=params_msml1h.log_dir)
    ...
    train_loader = init_data_loader(
        train_dataset,
        validation=False,
        multilingual=params_msml1h.separate_files,
    )
    ...
    optimizer = torch.optim.Adam(
        params=model.parameters(), lr=params_msml1h.learning_rate
    )
...
if __name__ == "__main__":
    torch.manual_seed(params_msml1h.random_seed)
    np.random.seed(params_msml1h.random_seed)
